...
def epanechnikov_density(X, sigmaopt, lambdaopt, dt, verbose=False, BallTree=True, chunks=1, weights=None):
    ...
    for X_i, lambda_i in zip(np.array_split(X, chunks), np.array_split(lambdaopt, chunks)):
        # Obtain Euclidean distances to all points within the kernel bandwidth for each point 
        if BallTree:
                inds, dists = dt.query_radius(X_i,(sigmaopt*lambda_i),return_distance=True)
        else:
                dist, ind = dt.query(X_i,N,distance_upper_bound=sigmaopt*np.max(lambdaopt))
        ...
        t_dot_t = [ (d/(sigmaopt*l))**2 for d, l in zip(dists, lambda_i) ]
        ...
        for t, l, w in zip(t_dot_t, lambda_i, ws):
            ...
            if weights is not None:
                rho = (1./N) *  np.sum([ x * w for x, w in zip( (sigmaopt*l)**(-ndim)*Ke, w[t < 1] ) ])
            else:
                rho = (1./N) * np.sum((sigmaopt*l)**(-ndim)*Ke)
            rhos.append(rho)
    return np.asarray(rhos)

def modified_breiman_density(X, verbose=False, BallTree=True, chunks=1, weights=None):
    '''
    purpose:
    ...
    if verbose: print('Calculating pilot densities with fixed bandwidth...')
    pilot_rho = epanechnikov_density(X, sigmaopt, lambdaopt, dt, BallTree=BallTree, chunks=chunks, weights=weights)

    # (3) Calculate the new local bandwith for each point
    if verbose: print('Calculating densities using variable bandwidth from pilot densities...')
    g = np.exp(np.sum(np.log(pilot_rho)/X.shape[0]))
    lambdaopt_new = (pilot_rho/g)**(-1./X.shape[1])

    # (4) Use new bandwidths to calculate density
    rho = epanechnikov_density(X, sigmaopt, lambdaopt_new, dt, BallTree=BallTree, chunks=chunks, weights=weights)
